...
export function DashboardSettingsModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 font-sans">
      <div className={`rounded-none p-6 max-w-md w-full space-y-6 shadow-2xl border ${
        isDark ? "bg-[#0a0a0a] border-[#1f1f1f]" : "bg-white border-slate-200 text-slate-900"
      }`}>
        <div className={`flex items-center justify-between border-b pb-4 ${isDark ? "border-[#1f1f1f]" : "border-slate-200"}`}>
          <h3 className={`text-lg font-bold flex items-center gap-2 ${isDark ? "text-[#ededed]" : "text-slate-900"}`}>
            <Settings className="w-5 h-5 text-white" /> Dashboard Settings
          </h3>
          <button onClick={onClose} className={`cursor-pointer ${isDark ? "text-[#a1a1a1] hover:text-white" : "text-slate-500 hover:text-slate-900"}`}>
        ...
        <div className="space-y-2">
          <label className={`text-xs font-semibold block ${isDark ? "text-[#a1a1a1]" : "text-slate-700"}`}>Auto-Refresh Interval</label>
        ...
        <div className="space-y-3">
          <label className={`text-xs font-semibold block ${isDark ? "text-[#a1a1a1]" : "text-slate-700"}`}>Widget Layout & Visibility</label>
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1 text-xs">
            {Object.entries(widgetVisibility).map(([key, isVisible]) => (
              <div key={key} className={`flex items-center justify-between p-2.5 rounded-none border ${
                isDark ? "bg-[#111111] border-[#1f1f1f]" : "bg-slate-50 border-slate-200"
              }`}>
                <span className={`capitalize ${isDark ? "text-[#ededed]" : "text-slate-800"}`}>{key.replace(/([A-Z])/g, " $1")}</span>
                <button

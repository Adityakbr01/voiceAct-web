...
export function CampaignAnalyticsTable({
  ...
  return (
    <section
      className={`p-7 rounded-none border shadow-sm space-y-6 font-sans ${
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative">
          <Search
            className={`w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
          />
          <input
            type="text"
      ...
      {/* Table */}
      <div
        className={`overflow-x-auto rounded-none border ${isDark ? "border-[#1f1f1f]" : "border-slate-200"}`}
      >
          ...
          >
            {filteredCampaigns.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className={`p-6 text-center ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
                >
              ...
              filteredCampaigns.map((cmp) => (
                <tr
                  key={cmp.id}
                  className={`transition ${isDark ? "hover:bg-[#111111]" : "hover:bg-slate-50"}`}
                >
                  <td
                    className={`px-5 py-4 font-bold ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                  >
                    {cmp.name}
                  </td>
                  <td
                    className={`px-5 py-4 font-medium ${isDark ? "text-[#a1a1a1]" : "text-slate-600"}`}
                  >
                    {cmp.source} / {cmp.medium}
                  </td>
                  <td
                    className={`px-5 py-4 font-mono font-semibold ${isDark ? "text-[#ededed]" : "text-slate-700"}`}
                  >
                    {cmp.clicks.toLocaleString()}
                  </td>
                  <td
                    className={`px-5 py-4 font-mono font-extrabold ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                  >
                    {cmp.conversions}
                  </td>

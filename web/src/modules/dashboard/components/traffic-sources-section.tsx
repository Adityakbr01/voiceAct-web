...
export function TrafficSourcesSection({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-sans">
      ...
      <div
        className={`p-7 rounded-none space-y-5 border shadow-sm ${
      ...
      >
        ...
        <p className={`text-xs ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}>
          Distribution across Direct, Search, Paid, Social, and Referrals.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 items-center pt-2">
          ...
          <div className="space-y-2.5 max-h-64 overflow-y-auto pr-2">
            {sanitizedSources.map((src) => (
              <div
                key={src.name}
                className={`flex items-center justify-between p-3.5 px-4 rounded-none text-xs border ${
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  />
                  <span className={`font-semibold ${isDark ? "text-[#ededed]" : "text-slate-800"}`}>
                    {src.name}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={`font-mono font-bold ${isDark ? "text-[#ededed]" : "text-slate-700"}`}
                  >
                    {src.visitors.toLocaleString()}
                  </span>
                  <span
                    className={`text-[10px] font-semibold w-10 text-right ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
                  >
                    {src.percentage}%
                  </span>
                </div>
      ...
      >
        ...
        <p className={`text-xs ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}>
          External domains driving qualified visitors.
        </p>

        <div className="space-y-3 pt-2">
          {sources.length === 0 ? (
            <div
              className={`p-6 text-center text-xs ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
            >
            ...
            sources.map((ref) => (
              <div
                key={ref.name}
                className={`p-4 px-5 rounded-none flex items-center justify-between text-xs border ${
                  isDark ? "bg-[#111111] border-[#1f1f1f]" : "bg-slate-50 border-slate-200"
                }`}
              >
                <div>
                  <div className={`font-bold ${isDark ? "text-[#ededed]" : "text-slate-800"}`}>
                    {ref.name}
                  </div>
                  <div
                    className={`text-[10px] mt-0.5 font-medium ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
                  >
                    Organic / Direct
                  </div>
                </div>
                <div className="text-right">
                  <div
                    className={`font-bold font-mono ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                  >
                    {ref.visitors.toLocaleString()} vis
                  </div>
                  <div
                    className={`text-[10px] font-semibold ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
                  >
                    {ref.percentage}%
                  </div>
                </div>

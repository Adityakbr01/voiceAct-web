...
export function ConversionFunnelSection({
  ...
  return (
    <section ref={funnelRef} className="grid grid-cols-1 xl:grid-cols-3 gap-6 font-sans">
      {/* Lead Metrics Cards */}
      <div
        className={`xl:col-span-1 p-7 rounded-none space-y-5 border shadow-sm ${
      ...
      >
        ...
        <p className={`text-xs ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}>
          Real contact submissions from website.
        </p>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2">
          {leadMetrics.map((metric) => (
            ...
            >
              <span
                className={`text-[11px] font-bold block ${isDark ? "text-[#a1a1a1]" : "text-slate-600"}`}
              >
                {metric.title}
              </span>
              <span
                className={`text-2xl font-extrabold font-mono mt-1.5 block ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
              >
                {metric.count}
              </span>
            </div>
      ...
      <div
        className={`xl:col-span-2 p-7 rounded-none space-y-6 border shadow-sm ${
          isDark ? "bg-[#0a0a0a] border-[#1f1f1f]" : "bg-white border-slate-200"
        }`}
      >
        <div>
          ...
          <p className={`text-xs mt-1 ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}>
            Live visitor conversion progression and drop-off analysis.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {defaultFunnel.map((step, idx) => (
            <div
              key={step.step}
              className={`p-4 rounded-none border space-y-2.5 ${
                isDark ? "bg-[#111111] border-[#1f1f1f]" : "bg-slate-50 border-slate-200"
              }`}
            >
              <div className="flex items-center justify-between text-xs">
                <span className={`font-bold ${isDark ? "text-[#ededed]" : "text-slate-800"}`}>
                  {step.step}
                </span>
                <div className="flex items-center gap-3">
                  <span
                    className={`font-mono font-bold ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                  >
              ...
              <div
                className={`w-full rounded-none h-3.5 overflow-hidden border ${

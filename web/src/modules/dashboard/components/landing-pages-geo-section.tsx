...
export function LandingPagesGeoSection({
  ...
  return (
    <div ref={containerRef} className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-sans">
      {/* Top Landing Pages */}
      <div
        className={`p-6 rounded-none space-y-4 border ${
            ...
            >
              {landingPages.length === 0 ? (
                <tr>
                  <td
                    colSpan={2}
                    className={`p-4 text-center ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
                  >
                ...
                landingPages.map((page) => (
                  <tr
                    key={page._id}
                    className={`landing-row transition ${isDark ? "hover:bg-[#111111]" : "hover:bg-slate-50"}`}
                  >
                    <td
                      className={`p-2.5 font-mono font-semibold ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                    >
                      {page._id || "/"}
                    </td>
                    <td
                      className={`p-2.5 text-right font-mono ${isDark ? "text-[#ededed]" : "text-slate-700"}`}
                    >
      ...
      >
        ...
        <div className="space-y-3">
          {countries.length === 0 ? (
            <div
              className={`p-4 text-center text-xs ${isDark ? "text-[#a1a1a1]" : "text-slate-500"}`}
            >
            ...
            countries.map((geo) => (
              <div
                key={geo._id}
                className={`geo-card-item p-3 rounded-none flex items-center justify-between text-xs transition-all duration-200 border ${
                  isDark
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span
                      className={`font-semibold block ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                    >
                      {geo._id || "Unknown"}
                    </span>
                  </div>
                ...
                <div className="text-right">
                  <span
                    className={`font-bold font-mono block ${isDark ? "text-[#ededed]" : "text-slate-900"}`}
                  >
                    {geo.count.toLocaleString()} visitors
                  </span>
                </div>
              </div>
            ))
